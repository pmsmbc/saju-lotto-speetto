...
const rounds = [
  ...
  { game: '스피또2000', gameCode: 'SP2000', round: 65, status: '판매종료', rank1Remaining: 1, rank1Total: 6 },
  { game: '스피또2000', gameCode: 'SP2000', round: 64, status: '판매종료', rank1Remaining: 0, rank1Total: 6 },
...
test('판매종료 회차는 최신순으로 최대 2개만', () => {
  expect(recentFinished(rounds, 'SP2000').map((r) => r.round)).toEqual([66, 65])
})

test('limit을 넘겨 개수를 조절할 수 있음', () => {
  expect(recentFinished(rounds, 'SP2000', 1).map((r) => r.round)).toEqual([66])
})
