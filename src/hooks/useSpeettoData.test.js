...
describe('useSpeettoData', () => {
  ...
  test('성공 시 rounds와 stores 반환', async () => {
    const data = {
      updatedAt: '2026-07-01T00:00:00Z',
      rounds: [{ gameCode: 'SP2000', round: 68 }],
      stores: [{ game: '스피또2000', round: 68, rank: 1, store: 'A', address: '서울', region: '서울' }],
    }
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ ok: true, json: async () => data }))
    const { result } = renderHook(() => useSpeettoData())
    await waitFor(() => expect(result.current.loading).toBe(false))
    expect(result.current.rounds).toEqual(data.rounds)
    expect(result.current.stores).toEqual(data.stores)
    expect(result.current.updatedAt).toBe('2026-07-01T00:00:00Z')
    expect(result.current.error).toBeNull()
  })

  test('실패 시 에러 메시지, rounds=[], stores=[]', async () => {
